# HearthBit documentation

## Core documentation by language

- English: [project overview](../README.md) and
  [transparency](transparency.md).
- Español: [descripción del proyecto](../README.es.md) y
  [transparencia](transparency.es.md).
- Deutsch: [Projektübersicht](../README.de.md) und
  [Transparenz](transparency.de.md).
- Français : [présentation du projet](../README.fr.md) et
  [transparence](transparency.fr.md).
- 简体中文：[项目概述](../README.zh.md)和[透明度](transparency.zh.md)。
- 日本語：[プロジェクト概要](../README.ja.md)と
  [透明性](transparency.ja.md)。

Technical specifications use English or Spanish according to their original
maintainer. Protocol identifiers, byte layouts and source-code symbols are not
translated because they must remain interoperable.

Translations must preserve security warnings, limitations and licensing
meaning. When translations disagree, the English license text in
[`LICENSE`](../LICENSE) controls.

## Important technical documents

- [Architecture](architecture.md)
- [Protocol](protocol.md)
- [BitChat core profile](bitchat-core-profile.md)
- [Extension registry](extension-registry.md)
- [Transfer protocol](transfer-protocol.md)
- [Radar and ranging validation](radar-ranging-validation.md)
- [Field testing](field-test.md)
- [Battery and memory validation](battery-memory-validation.md)
- [Localization workflow](localization.md)
- [Security reporting](../SECURITY.md)
- [Contribution policy](../CONTRIBUTING.md)

## Component guides

- Mobile app: [English](../app/README.md) ·
  [Español](../app/README.es.md)
- Linux/Raspberry Pi relay: [English](../relay/README.md) ·
  [Español](../relay/README.es.md)
- Anchor firmware: [English](../firmware/README.md) ·
  [Español](../firmware/README.es.md)
- Pinned reference dependencies: [English](../vendor/README.md) ·
  [Español](../vendor/README.es.md)
- Binary conformance suite:
  [English](../tests/conformance/README.md) ·
  [Español](../tests/conformance/README.es.md)

Documentation corrections are welcome under the same license that applies to
original HearthBit code. Third-party excerpts retain their original terms.
